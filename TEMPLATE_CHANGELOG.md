...
- Added: downstream patch `patches/KIN-4699-decisions-inbox.patch`, applied at build time on top of pinned upstream `v2026.609.0` (after `KIN-4697`). The UI slice of the approved KIN-4606 "Better Actions" plan (Phase 1), built on the KIN-4697 `pending-interactions` endpoint. Adds a pinned **Decisions** section at the top of the inbox "mine" tab (`DecisionsSection`/`DecisionInboxRow`, fed by `issuesApi.listPendingInteractions`): an oldest-first (FIFO) list of pending board decisions with a count and collapse state persisted under the existing `collapsedGroupKeys` (`decisions` key). Plain yes/no `request_confirmation`s (no document target, no details body) resolve **inline** with Accept/Reject — required-reason declines expand a textarea in the row. Everything longer opens the **Quick view slide-over** (`Sheet`, right): compact issue context + "Open in issue" deep link to `#interaction-<id>`, the real `IssueThreadInteractionCard` rendered in place (single source of truth — same accept/reject/respond/cancel routes and continuation wakes as the thread), and the targeted plan/document rendered via `MarkdownBody` for plan-bound confirmations. The inbox sidebar badge count now includes pending decisions. UI-only and additive: no new server routes, no schema change; reuses existing interaction mutations. Per-user "for me" scoping (responder binding) is Phase 3 (KIN-4701) — until then the list is company-wide pending. Verified `git apply` clean against pristine `v2026.609.0` (and layered after KIN-4355 + KIN-4697); `pnpm --filter @paperclipai/ui typecheck` (`tsc -b`) passes. Recommend upstreaming to `paperclipai/paperclip` and dropping the patch once a release contains it. See KIN-4699 (parent KIN-4606).
